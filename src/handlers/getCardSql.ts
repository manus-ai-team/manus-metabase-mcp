...
export async function handleGetCardSql(
  ...
  try {
    ...
    throw handleApiError(error, {
      operation: 'Fetch card SQL details',
      resourceType: 'card',
      resourceId: cardId,
      customMessages: {
        '400': 'Invalid card_id parameter. Ensure the card ID is a valid number.',
        '404': 'Card not found. Check that the card_id is correct and the card exists.',
        '500': 'Metabase server error. The card may be corrupted or the server is experiencing issues.'
      }
    }, logError);
  }
}
